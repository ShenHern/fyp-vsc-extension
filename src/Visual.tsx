...
import { format } from "./utils/jsonParser";
...
function RenderGraph(props : any) {
  ...
  window.addEventListener("message", (event) => {
    if(event.data.command==="traceArray") {
      setTrace(event.data.payload);
    } else if (event.data.command==="formatNodes") {
    ...
    <div>
      <div className={styles.select}>
        ...
        trace.map((trace, index) => {
            return(
            <option key={index} value={JSON.stringify(trace)}>Time: {trace[0]}&nbsp;&nbsp;&nbsp;&nbsp;{trace[1]}&nbsp;&nbsp;&nbsp;&nbsp;({Object.keys(trace[2]).length} events)</option>
            )
        })
        }
        </select>
